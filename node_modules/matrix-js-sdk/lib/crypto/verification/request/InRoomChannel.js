...
class InRoomChannel {
  ...
  static validateEvent(event, client) {
    ...
    if (type === _VerificationRequest.REQUEST_TYPE) {
      ...
      const ownUserId = client.getUserId(); // ignore requests that are not direct to or sent by the syncing user
    ...
    return _VerificationRequest.VerificationRequest.validateEvent(type, event, InRoomChannel.getTimestamp(event), client);
  }
  /**
  ...
  async handleEvent(event, request) {
    ...
    if (!this._requestEventId && type === _VerificationRequest.REQUEST_TYPE) {
